...
const HighlightsStrip = () => {
  ...
  const { gridClassName, maxWidthClassName } = getAlbumGridLayout(slides.length);
    ...
    <section className="bg-[#09090B] px-4 pb-12 sm:px-6 sm:pb-16 md:px-8">
      <div className="mx-auto max-w-5xl">
        <div className="mb-4 flex items-center justify-between gap-4">
          <h2 className="text-[clamp(1.4rem,3vw,2rem)] font-semibold tracking-tight text-white">Highlights</h2>
        ...
        <div className={`mx-auto gap-2 ${gridClassName} ${maxWidthClassName}`} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setPaused(false); }}>
          {slides.map((photoIndex, slot) => {
            const photo = photos[photoIndex];
            if (!photo) return null;
            return (
              <div key={slot} className="mb-2 break-inside-avoid min-w-0">
                <div className="relative overflow-hidden rounded-lg bg-white/5">
                  <AnimatePresence initial={false} mode="wait">
                    <motion.button
                      ...
                      className="group relative block w-full cursor-pointer overflow-hidden p-0 text-left focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white/70"
                    ...
                    >
                      <img src={getPreviewImageUrl(photo.thumbnailUrl) ?? photo.thumbnailUrl ?? photo.url} alt={clampCaption(photo.caption) ?? photo.fileName} className="block h-auto w-full object-contain transition-transform duration-500 group-hover:scale-[1.03]" loading={slot < 3 ? "eager" : "lazy"} />
                    </motion.button>
                  </AnimatePresence>
                </div>
